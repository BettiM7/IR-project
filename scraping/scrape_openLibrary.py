...
from scraping.scrape_links import scrape_links
...
if __name__ == "__main__":
    """
    This script automates the process of scraping book links and details from the Open Library website for a specified subject and page range.

    Description:
        - Defines a range of pages to scrape and constructs a list of URLs for the specified subject, inserted manually.
        - Generates a unique filename prefix based on the domain, subject, and page range for organizing output files.
        - Ensures directories for storing scraped URLs and data exist, creating them if necessary.
        - Checks for previously scraped links in the designated directory and avoids duplicating scraping efforts.

        Scraping Workflow:
        - Iterates through the URLs, scraping book links using CSS selectors (initially targeting `<h3>` tags with class `booktitle`).
        - Retries scraping with a higher timeout and a different selector (`div` with class `preview-name`) if no links are found.
        - Filters out already-scraped links and updates the list of new links.
        - Saves all unique scraped links to a `.txt` file for future reference.

        Data Extraction:
        - Reads existing scraped book data from a JSON file, if available, to avoid duplicating work.
        - Iterates through the scraped links, extracting book details and saving them incrementally to a JSON file.
        - Tracks progress, showing completion percentage and estimated time remaining.

        Retry Logic:
        - If scraping fails for certain links, retries them with a higher timeout.
        - Logs and saves any links that still fail to scrape after retries to a separate JSON file.

    Output:
        - A `.txt` file containing all successfully scraped book links.
        - A `.json` file containing detailed data for each book, incrementally updated during the scraping process.
    ...
    urls = [
               ...
               f"https://openlibrary.org/search?q=subject%3A%22Computer+science%22&sort=editions&subject_facet=Computer+science&page={x}"
               for x in
