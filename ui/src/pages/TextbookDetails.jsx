...
export default function TextbookDetails() {
  ...
  const { title = [""], subtitle = [""], publish_date, authors = [], subjects = [], image, description = [], publisher = [], language, isbn13, source } = data;
    ...
    <>
      <div className="max-w-[60vw] mx-auto bg-white shadow-lg rounded-lg p-6 mt-2 mb-2">
        <div className="flex items-start gap-6">
          {isImageValid && image ? (
            <img className="w-40 h-auto rounded-lg shadow-md" src={image} alt={title[0]} onError={() => setIsImageValid(false)} />
          ) : (
            <div>
              {image ? (
                <div className="flex flex-col w-40 h-40 items-center justify-center bg-gray-100 text-gray-500 rounded-lg shadow-md">
          ...
          <div className="flex-1">
            ...
            <div className="mt-4">
              ...
              {description.length > 0 ? <p className=" text-justify text-gray-600 mt-2">{description.join(" ")}</p> : <p className="text-gray-500">No description available</p>}
            </div>
          </div>
        </div>
