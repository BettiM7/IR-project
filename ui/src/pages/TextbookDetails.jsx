                ...
                {isImageValid && image ? (
                    ...
                    <div>
                        {image ? (<div
                            ...
                            <span className="text-sm">Could not load source</span></div>) : (
                            <div
                                className="flex flex-col w-40 h-40 items-center justify-center bg-gray-100 text-gray-500 rounded-lg shadow-md">
                                <TbError404 className={"text-2xl"}/>
                                <span className="text-sm">No image available</span></div>)}
                    </div>
                )}
                    ...
                    {authors.length > 0 ? (
                        <div className="text-gray-600 mt-2">
                            {authors.map((author, index) => (
                                <div className={"mb-1"} key={index}>{author}</div>
                            ))}
                        </div>
                    ) : (
                ...
                <div>
                    ...
                    <div className=" text-gray-600 mt-2 pl-4 border-gray-300">
                        {subjects.map((subject, index) => (
                            <div className={"mb-1"} key={index}>{subject}</div>
                        ))}
                    </div>
                </div>
