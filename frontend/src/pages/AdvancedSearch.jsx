...
export default function AdvancedSearch() {
    ...
    function validateAndSubmitNarrow() {
        const hasNullTermFields = queryTerms.some((term) => term.field === null || term.term === null);
        const hasNullRestrictFields = !startYear && !endYear && !isbn && !selectedLanguage;
        if (hasNullTermFields && hasNullRestrictFields) {
            toast.current.show({
                severity: "error",
                summary: "Validation Error",
                detail: "No complete search queries were given",
                life: 3000,
            });
            return;
        }

        const queryTermsPart = queryTerms
            .map((obj, index) => {
                const operator = index > 0 ? `${obj.operator}` : "";
                return `${operator}(${obj.field}:${obj.term})`;
            })
            .join("");
        if (hasNullRestrictFields) {
            navigate(`/search?q=${queryTermsPart}`);
            return
        }
